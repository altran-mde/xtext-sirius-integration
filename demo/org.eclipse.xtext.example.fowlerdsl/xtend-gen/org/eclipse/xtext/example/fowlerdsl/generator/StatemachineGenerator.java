...
public class StatemachineGenerator implements IGenerator {
	...
	public void doGenerate(final Resource resource, final IFileSystemAccess fsa) {
		final String _className = this.className(resource);
		final String _plus = (_className + ".java");
		final EObject _head = IterableExtensions.<EObject> head(resource.getContents());
		fsa.generateFile(_plus, this.toJavaCode(((Statemachine) _head)));
	}
	
	public String className(final Resource res) {
		final String name = res.getURI().lastSegment();
	...
	public CharSequence toJavaCode(final Statemachine sm) {
		...
		final String _className = this.className(sm.eResource());
		...
		_builder.append("\t\t");
		_builder.append("new ");
		final String _className_1 = this.className(sm.eResource());
		...
		_builder.newLine();
		{
			final EList<Command> _commands = sm.getCommands();
			for (final Command c : _commands) {
				_builder.append("\t");
				final CharSequence _declareCommand = this.declareCommand(c);
		...
		final State _head = IterableExtensions.<State> head(sm.getStates());
		String _name = null;
		if (_head != null) {
			_name = _head.getName();
		}
		...
		_builder.newLine();
		{
			final EList<State> _states = sm.getStates();
			for (final State state : _states) {
				_builder.append("\t\t\t");
				final CharSequence _generateCode = this.generateCode(state);
				_builder.append(_generateCode, "\t\t\t");
				_builder.newLineIfNotEmpty();
			}
		}
		{
			final EList<Event> _resetEvents = sm.getResetEvents();
			for (final Event resetEvent : _resetEvents) {
				_builder.append("\t\t\t");
				_builder.append("if (\"");
				final String _name_1 = resetEvent.getName();
				...
				final State _head_1 = IterableExtensions.<State> head(sm.getStates());
				String _name_2 = null;
				if (_head_1 != null) {
	...
	public CharSequence declareCommand(final Command command) {
		...
		final String _firstUpper = StringExtensions.toFirstUpper(command.getName());
		_builder.append(_firstUpper);
		_builder.append("() {");
		_builder.newLineIfNotEmpty();
		_builder.append("\t");
		_builder.append("System.out.println(\"Executing command ");
		final String _name = command.getName();
		_builder.append(_name, "\t");
		_builder.append(" (");
		final String _code = command.getCode();
	...
	public CharSequence generateCode(final State state) {
		...
		final String _name = state.getName();
		_builder.append(_name);
		_builder.append("\")) {");
		_builder.newLineIfNotEmpty();
		_builder.append("\t");
		...
		_builder.newLine();
		{
			final EList<Command> _actions = state.getActions();
			for (final Command c : _actions) {
				_builder.append("\t\t");
				_builder.append("do");
				final String _firstUpper = StringExtensions.toFirstUpper(c.getName());
				_builder.append(_firstUpper, "\t\t");
				_builder.append("();");
				_builder.newLineIfNotEmpty();
			}
		...
		final String _name_1 = state.getName();
		_builder.append(_name_1, "\t");
		_builder.append("\'. Possible events are [");
		final Function1<Transition, String> _function = (final Transition t) -> {
			return t.getEvent().getName();
		};
		final String _join = IterableExtensions
				.join(ListExtensions.<Transition, String> map(state.getTransitions(), _function), ", ");
		...
		_builder.newLine();
		{
			final EList<Transition> _transitions = state.getTransitions();
			for (final Transition t : _transitions) {
				_builder.append("\t");
				_builder.append("if (\"");
				final String _name_2 = t.getEvent().getName();
				_builder.append(_name_2, "\t");
				_builder.append("\".equals(lastEvent)) {");
				_builder.newLineIfNotEmpty();
				_builder.append("\t");
				_builder.append("\t");
				_builder.append("currentState = \"");
				final String _name_3 = t.getState().getName();
