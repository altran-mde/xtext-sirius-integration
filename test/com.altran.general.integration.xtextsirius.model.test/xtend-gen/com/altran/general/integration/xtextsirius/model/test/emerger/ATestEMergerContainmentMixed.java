...
public abstract class ATestEMergerContainmentMixed<T extends IElement<T>> extends ATestEMergerContainment<T> {
  ...
  public void set_listEmpty() {
    ...
    final T result = this.createEMerger(existing, this.changeableUniqueListContFeature()).merge(Collections.<T>unmodifiableSet(CollectionLiterals.<T>newHashSet(_newEdited, _newEdited_1)), this.changeableUniqueListContFeature());
  ...
  public void set_listPartiallyExisting() {
    ...
    final T existing = ObjectExtensions.<T>operator_doubleArrow(_createRootElement_1, _function_1);
    T _head = IterableExtensions.<T>head(edited.getChangeableUniqueListCont());
