...
public class TestEMergerContainmentKey extends ATestEMergerContainmentIdentity<KeyElement> {
  ...
  public void set_listPartiallyExisting() {
    ...
    final KeyElement existing = ObjectExtensions.<KeyElement>operator_doubleArrow(_createRootElement_1, _function_1);
    KeyElement _head = IterableExtensions.<KeyElement>head(edited.getChangeableUniqueListCont());
    KeyElement _newEdited = this.newEdited(2, "2");
    final KeyElement result = this.createEMerger(existing, this.changeableUniqueListContFeature()).merge(Collections.<KeyElement>unmodifiableSet(CollectionLiterals.<KeyElement>newHashSet(_head, _newEdited)), this.changeableUniqueListContFeature());
